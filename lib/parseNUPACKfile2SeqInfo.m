function [seq_info,total_defect] = parseNUPACKfile2SeqInfo(file_name)
    seq_info = {};
    FLAG = 1;
    ...
    while ~feof(fid)        
        tline = fgetl(fid);
        name_index1 = strfind(tline,'defect:');
        if ~isempty(name_index1)
            total_defect = str2num(tline(length('defect:')+name_index1:end));
            break;
        end
    end
